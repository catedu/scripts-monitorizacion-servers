...
do
    ...
    for detalle in $( docker exec -ti docker-backup_db_1 mysql -u ${USERDB} -p${PASSDB} -N -e "select REPLACE(REPLACE(message,'\"','\''),'/',' per ') from elkarbackup.LogRecord where datetime like '$date%' and link = '$link' and level > 100 and source = 'RunJobCommand' AND message like '%Command%' " )
    do
